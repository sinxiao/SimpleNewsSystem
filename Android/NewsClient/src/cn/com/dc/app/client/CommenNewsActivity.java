...
import cn.com.dc.app.client.bean.NewsBean;
...
public class CommenNewsActivity extends DCActivity {
	...
	private OnItemLongClickListener newsLongClick = new OnItemLongClickListener() {
		...
		public boolean onItemLongClick(AdapterView<?> views, View view,
				int idx, long itemId) {
			PopupWindow popup = Utils.getPopupWindow(CommenNewsActivity.this,
					R.layout.popup_menu);
	...
	private void bindNewsData() {
		if (lsVNews != null && galyNews != null) {
			if (newsAdapter == null) {
				...
				newsAdapter = new NewsAdapter(listNews, this, mHandler);
...
//				ArrayAdapter<NewsBean> ader = new ArrayAdapter<NewsBean>(this, resource, textViewResourceId, objects);
	...
	private OnItemClickListener newsClick = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> views, View view, int idx,
			...
			if (!click) {
				click = true;
				Intent intent = new Intent(CommenNewsActivity.this,
		...
		public void handleMessage(android.os.Message msg) {
			switch (msg.what) {
				case HANDLER_SHOWERROR :
					...
					dismissWait();
					Toast.makeText(CommenNewsActivity.this, msg.arg1,
				...
				case HANDLER_NOMORE :
					...
					// if (nowpage==news_type) {
					dismissWait();
					Toast.makeText(CommenNewsActivity.this, R.string.tip_error,
							Toast.LENGTH_SHORT).show();
					// }
					break;
				case HANDLER_END :
